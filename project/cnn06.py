...
def model_fn(features, labels, mode):
    ...
    embeddings = tf.get_variable("embeddings", [VOCAB, EMBED], tf.float32)
    ...
    x = tf.layers.dense(x, EMBED)
    x = tf.reshape(x, [-1, MAX_OUTPUT_LEN, EMBED])
    output = x
    # =================== Model Ends Here
...
def modelSpec(mode, output, labels, lens):
    if mode == tf.estimator.ModeKeys.PREDICT:
        labels = tf.reshape(labels, [-1, MAX_OUTPUT_LEN, EMBED])
        labels = tf.nn.l2_normalize(labels, -1)
        output = tf.nn.l2_normalize(output, -1)

        loss = tf.losses.cosine_distance(
            labels,
            tf.contrib.seq2seq.tile_batch(output, 4),
            -1,
        ...
        labels = tf.nn.l2_normalize(labels, -1)
        output = tf.nn.l2_normalize(output, -1)
        loss = tf.losses.cosine_distance(labels, output, -1)
