...
def model_fn(features, labels, mode):
    ...
    x1 = convblock(x, 32, 8, 1, training=training)

    x = convblock(x1, 128, 8, 1, training=training)
    x = convblock(x, 256, 8, 2, training=training)
    x2 = convblock(x, 64, 8, 1, training=training)

    x = convblock(x2, 256, 8, 1, training=training)
    x = convblock(x, 512, 8, 2, training=training)
    x3 = convblock(x, 128, 8, 1, training=training)

    x = convblock(x3, 512, 8, 1, training=training)
    x = convblock(x, 1024, 8, 2, training=training)
    x4 = convblock(x, 256, 8, 1, training=training)

    x = tf.concat([
        tf.reshape(x1, [-1, 16, 256]),
        tf.reshape(x2, [-1, 16, 256]),
        tf.reshape(x3, [-1, 16, 256]), x4
    ], -1)
    print(x)
